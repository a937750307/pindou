...
function CanvasEngine(canvas, grid, palette) {
  ...
  this.dotMode = false;
  this.showGrid = true;
  this.gridLineColor = 'white'; // 九宫格线颜色：white/black/red/blue

  // 编辑模式
  this.editMode = false;
  this.selectedColorIndex = 0; // 当前选中的颜色索引
  this.hoveredCell = null; // {col, row} 当前悬停的格子
...
CanvasEngine.prototype.render = function() {
  ...
  if (this.showGrid) {
    ...
    ctx.strokeStyle = lineColors[this.gridLineColor] || lineColors.white;
  ...
  // 编辑模式：悬停高亮
  if (this.editMode && this.hoveredCell) {
    var hx = this.offsetX + this.hoveredCell.col * bs;
    var hy = this.offsetY + this.hoveredCell.row * bs;
    ctx.strokeStyle = 'rgba(255, 255, 0, 0.8)';
    ctx.lineWidth = Math.max(this.scale * 0.8, 2);
    ctx.strokeRect(hx + 1, hy + 1, bs - 2, bs - 2);
  }
};
...
CanvasEngine.prototype._bindEvents = function() {
  ...
  // 编辑模式：mousemove 计算悬停格子
  canvas.addEventListener('mousemove', function(e) {
    if (self.editMode) {
      var rect = canvas.getBoundingClientRect();
      var mx = e.clientX - rect.left;
      var my = e.clientY - rect.top;
      var bs = self.beadSize * self.scale;
      var col = Math.floor((mx - self.offsetX) / bs);
      var row = Math.floor((my - self.offsetY) / bs);
      if (col >= 0 && col < self.grid.cols && row >= 0 && row < self.grid.rows) {
        if (!self.hoveredCell || self.hoveredCell.col !== col || self.hoveredCell.row !== row) {
          self.hoveredCell = { col: col, row: row };
          self.render();
        }
      } else if (self.hoveredCell) {
  ...
  canvas.addEventListener('click', function(e) {
    ...
    var col = Math.floor((mx - self.offsetX) / bs);
    var row = Math.floor((my - self.offsetY) / bs);
    if (col < 0 || col >= self.grid.cols || row < 0 || row >= self.grid.rows) return;

    // 触发编辑请求事件，让 UI 层弹出色号输入框
    var event = new CustomEvent('editCellRequest', {
      detail: { col: col, row: row }
    });
    self.canvas.dispatchEvent(event);
  });

  // 编辑模式：mouseleave 清除悬停
  canvas.addEventListener('mouseleave', function() {
    if (self.editMode && self.hoveredCell) {
      self.hoveredCell = null;
      self.render();
    }
  });
